...
import { Search, Filter, MapPin, Phone, Mail, Building2, Clock, Star, Calendar, Users } from 'lucide-react';
...
import HospitalBookingModal from '@/components/hospitals/HospitalBookingModal';

const HospitalDirectory = () => {
  const [hospitals, setHospitals] = useState<Hospital[]>([]);
  const [filteredHospitals, setFilteredHospitals] = useState<Hospital[]>([]);
  const [hospitalDoctors, setHospitalDoctors] = useState<Record<string, number>>({});
  ...
  const loadHospitalDoctorCounts = async () => {
    try {
      const doctorCounts: Record<string, number> = {};
      
      for (const hospital of hospitals) {
        try {
          const doctors = await hospitalManagementService.getHospitalDoctors(hospital.id);
          doctorCounts[hospital.id] = doctors.length;
        } catch (error) {
          console.error(`Error loading doctors for hospital ${hospital.id}:`, error);
          doctorCounts[hospital.id] = 0;
        }
      }
      
      setHospitalDoctors(doctorCounts);
    } catch (error) {
      console.error('Error loading hospital doctor counts:', error);
    }
  };
              ...
              <Card key={hospital.id} className="hover:shadow-lg transition-shadow border-l-4 border-l-teal-500">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 text-sm text-blue-600">
                    ...
                    <span>{hospitalDoctors[hospital.id] || 0} available doctors</span>
                  </div>
                  ...
                  <div className="flex gap-2 pt-4">
                    ...
                    <Button 
                      ...
                      onClick={() => setSelectedHospital({id: hospital.id, name: hospital.name})}
                    >
