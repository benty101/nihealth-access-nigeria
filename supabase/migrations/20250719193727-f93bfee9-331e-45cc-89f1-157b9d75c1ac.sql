...
-- Fix critical medical_records security vulnerability
-- Ensure user_id is NOT NULL to prevent RLS bypass
...
-- Ensure user_id is NOT NULL constraint exists
DO $$
BEGIN
    ALTER TABLE public.medical_records ALTER COLUMN user_id SET NOT NULL;
EXCEPTION
    WHEN others THEN
        -- Column might not exist yet or already has constraint
        NULL;
END $$;
...
-- Create new secure RLS policies for medical_records
