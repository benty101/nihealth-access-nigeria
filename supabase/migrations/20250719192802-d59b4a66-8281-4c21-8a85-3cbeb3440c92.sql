...
-- Phase 1: Fix Database Function Security and Critical RLS Issues
...
-- 2. Fix critical RLS vulnerability: Create medical_records table if not exists with proper constraints
CREATE TABLE IF NOT EXISTS public.medical_records (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, -- This MUST be NOT NULL for RLS security
...
DROP POLICY IF EXISTS "Healthcare providers can view medical records" ON public.medical_records;
DROP POLICY IF EXISTS "Admins can manage all medical records" ON public.medical_records;

-- Create secure RLS policies for medical_records
...
CREATE POLICY "Healthcare providers can view medical records"
ON public.medical_records
FOR SELECT
USING (
...
-- 3. Clean up redundant RLS policies on user_roles table
